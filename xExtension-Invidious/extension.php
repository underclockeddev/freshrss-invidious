 ...
 *
 * Latest version can be found at https://github.com/underclockeddev/freshrss-invidious
 *
...
class InvidiousExtension extends Minz_Extension
{
    ...
    public function convertYoutubeFeedUrl($url)
    {
        ...
        if (preg_match('#^https?://inv\.tux\.pizza/channel/([0-9a-zA-Z_-]{6,36})#', $url, $matches) === 1) {
            return 'https://inv.tux.pizza/feeds/videos.xml?channel_id=' . $matches[1];
        }

        if (preg_match('#^https?://inv\.tux\.pizza/user/([0-9a-zA-Z_-]{6,36})#', $url, $matches) === 1) {
            return 'https://inv.tux.pizza/feeds/videos.xml?user=' . $matches[1];
        }
    ...
    public function embedYouTubeVideo($entry)
    {
        ...
        if (preg_match('#^https?://inv\.tux\.pizza/watch\?v=|/videos/watch/[0-9a-f-]{36}$#', $link) !== 1) {
            return $entry;
        }
        ...
        if (stripos($link, 'inv.tux.pizza/watch?v=') !== false) {
    ...
    public function getHtmlContentForLink($entry, $link)
    {
        ...
        $url = str_replace('//inv.tux.pizza/watch?v=', '//'.$domain.'/embed/', $link);
